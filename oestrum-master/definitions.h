...
{
	int shotsFired; //Per player shots fired by aimbot
	int shotsHit; //Per player shots hit by aimbot
	int shotsMissed; //Per player shots missed by aimbot
	float lbyUpdateTime; //Set by resolver, predicted lby update time
	float lbyBacktrack; //Lby backtrack
	bool backtrackTick; //Backtrack tick
	float tickToBack; //Ticks to backtrack
	char* resolveType; //Player resolve type | 1 - back lby, 2 - air, 3 - moving, 4 - fakewalk, 5 - delta, 6 - less delta, 7 - lby

	PlayerSettings() : shotsFired(0), shotsHit(0), shotsMissed(0), lbyUpdateTime(0), lbyBacktrack(0), backtrackTick(false), tickToBack(0), resolveType("None") {}
}; extern struct PlayerSettings g_PlayerSettings[MAX_PLAYERS];
