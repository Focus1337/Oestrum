...
LRESULT __stdcall Hooks::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	}
	{
		...
		if (CurKey[g_Settings->misc_MenuKey])
		{
			isClicked = false;
			isDown = true;
		}
		else if (!CurKey[g_Settings->misc_MenuKey] && isDown)
		{
			isClicked = true;
			isDown = false;
		}
		...
		if (isClicked)
		{
			...
			cl_mouseenable->SetValue(!Global::Opened);
		}

	}
	if (Global::Init && Global::Opened && ImGui_ImplDX9_WndProcHandler(hWnd, uMsg, wParam, lParam))
		return true;
